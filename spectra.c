...
//    if (spec_BS_2_z2){for (i = 0; i < n_gal; i++){spec_BS_2_z2[i] = malloc(sizeof *spec_BS_2_z2[i] * n_E_gam);}}
    ...
    double **spec_prompt_MSP = malloc(sizeof *spec_prompt_MSP * n_gal);
    if (spec_prompt_MSP){for (i = 0; i < n_gal; i++){spec_prompt_MSP[i] = malloc(sizeof *spec_prompt_MSP[i] * n_E_gam);}}
    double **spec_IC_MSP_z1 = malloc(sizeof *spec_IC_MSP_z1 * n_gal);
    if (spec_IC_MSP_z1){for (i = 0; i < n_gal; i++){spec_IC_MSP_z1[i] = malloc(sizeof *spec_IC_MSP_z1[i] * n_E_gam);}}
    double **spec_SY_MSP_z1 = malloc(sizeof *spec_SY_MSP_z1 * n_gal);
    if (spec_SY_MSP_z1){for (i = 0; i < n_gal; i++){spec_SY_MSP_z1[i] = malloc(sizeof *spec_SY_MSP_z1[i] * n_E_gam);}}
    double **spec_BS_MSP_z1 = malloc(sizeof *spec_BS_MSP_z1 * n_gal);
    if (spec_BS_MSP_z1){for (i = 0; i < n_gal; i++){spec_BS_MSP_z1[i] = malloc(sizeof *spec_BS_MSP_z1[i] * n_E_gam);}}
    double **spec_IC_MSP_z2 = malloc(sizeof *spec_IC_MSP_z2 * n_gal);
    if (spec_IC_MSP_z2){for (i = 0; i < n_gal; i++){spec_IC_MSP_z2[i] = malloc(sizeof *spec_IC_MSP_z2[i] * n_E_gam);}}
    double **spec_SY_MSP_z2 = malloc(sizeof *spec_SY_MSP_z2 * n_gal);
    if (spec_SY_MSP_z2){for (i = 0; i < n_gal; i++){spec_SY_MSP_z2[i] = malloc(sizeof *spec_SY_MSP_z2[i] * n_E_gam);}}
    ...
    double **Q_e_MSP_z1 = malloc(sizeof *Q_e_MSP_z1 * n_gal);
    if (Q_e_MSP_z1){for (i = 0; i < n_gal; i++){Q_e_MSP_z1[i] = malloc(sizeof *Q_e_MSP_z1[i] * n_T_CR);}}
    double **Q_e_MSP_z2 = malloc(sizeof *Q_e_MSP_z2 * n_gal);
    if (Q_e_MSP_z2){for (i = 0; i < n_gal; i++){Q_e_MSP_z2[i] = malloc(sizeof *Q_e_MSP_z2[i] * n_T_CR);}}
    ...
    double **q_e_SS_MSP_z1 = malloc(sizeof *q_e_SS_MSP_z1 * n_gal);
    if (q_e_SS_MSP_z1){for (i = 0; i < n_gal; i++){q_e_SS_MSP_z1[i] = malloc(sizeof *q_e_SS_MSP_z1[i] * n_T_CR);}}
    double **q_e_SS_MSP_z2 = malloc(sizeof *q_e_SS_MSP_z2 * n_gal);
    if (q_e_SS_MSP_z2){for (i = 0; i < n_gal; i++){q_e_SS_MSP_z2[i] = malloc(sizeof *q_e_SS_MSP_z2[i] * n_T_CR);}}
    ...
    gsl_spline_object_1D De_gso1D_z1, De_gso1D_z2, gso_1D_Q_inject_1_z1, gso_1D_Q_inject_2_z1, gso_1D_Q_inject_1_z2, 
                         gso_1D_Q_inject_2_z2, gso_1D_Q_inject_MSP_z1, gso_1D_Q_inject_MSP_z2, qe_1_z1_so, qe_2_z1_so, qe_1_z2_so, 
                         qe_2_z2_so, qe_MSP_z1_so, qe_MSP_z2_so;
    ...
    #pragma omp parallel for schedule(guided) private(j, k, qe_1_z1_so, qe_2_z1_so, qe_1_z2_so, qe_2_z2_so, qe_MSP_z1_so, qe_MSP_z2_so, De_gso1D_z1, De_gso1D_z2, nphot_params, gso_1D_Q_inject_1_z1, gso_1D_Q_inject_2_z1, gso_1D_Q_inject_1_z2, gso_1D_Q_inject_2_z2, gso_1D_Q_inject_MSP_z1, gso_1D_Q_inject_MSP_z2, E_crit__GeV, seconds, gso2D_IC, gso2D_IC_Gamma, gso1D_fcal, t_loss_s ) firstprivate( gso2D_BS, gso1D_SY )
...
if (i == 10){Q_e_1_z1[i][j] = J( T_CR__GeV[j], Ce_Esm1[i], q_e_inject, m_e__GeV, T_e_cutoff__GeV ) * 0.1;}
        ...
        for (j = 0; j < n_T_CR; j++)
        {
            Q_e_MSP_z1[i][j] = d2NdEdt_MSP_e___GeVm1( T_CR__GeV[j], L_prompt__ergsm1( M_star__Msol[i] ) );
        }
        //spline object on total energy - not kinetic
        gso_1D_Q_inject_MSP_z1 = gsl_so1D( n_T_CR, E_CRe__GeV, Q_e_MSP_z1[i] );

        CRe_steadystate_solve_3( 1, E_CRe_lims__GeV, 500, n_H__cmm3[i], B__G[i], h__pc[i], 1, &gso2D_IC_Gamma, gso2D_BS, De_gso1D_z1, 
                               gso_1D_Q_inject_1_z1, gso_1D_Q_inject_2_z1, gso_1D_Q_inject_MSP_z1, &qe_1_z1_so, &qe_2_z1_so, &qe_MSP_z1_so );
...
//                               gso_1D_Q_inject_1_z1, gso_1D_Q_inject_2_z1, &qe_1_z1_so, &qe_2_z1_so );
        ...
        for (j = 0; j < n_T_CR; j++)
        {
            Q_e_MSP_z2[i][j] = gsl_so1D_eval( qe_MSP_z1_so, E_CRe__GeV[j] )/tau_diff__s( E_CRe__GeV[j], h__pc[i], De_gso1D_z1 );
        }
        //spline object on total energy - not kinetic
        gso_1D_Q_inject_MSP_z2 = gsl_so1D( n_T_CR, E_CRe__GeV, Q_e_MSP_z2[i] );

        CRe_steadystate_solve_3( 2, E_CRe_lims__GeV, 500, n_H__cmm3[i]/1000., B_halo__G[i], 50.*h__pc[i], 1, &gso2D_IC_Gamma, gso2D_BS,
                               De_gso1D_z2, gso_1D_Q_inject_1_z2, gso_1D_Q_inject_2_z2, gso_1D_Q_inject_MSP_z2, &qe_1_z2_so, &qe_2_z2_so, &qe_MSP_z2_so );
...
//        C_gam = norm_casc_C( spec_emit, E_GeV, n_Esteps, z[i], fdata_in );
        ...
        for (j = 0; j < n_E_gam; j++)
        {
            ...
            spec_prompt_MSP[i][j] = d2NdEdt_MSP_prompt___GeVm1( E_gam__GeV[j], L_prompt__ergsm1( M_star__Msol[i] ) );

            spec_IC_MSP_z1[i][j] = eps_IC_3( E_gam__GeV[j], gso2D_IC, qe_MSP_z1_so ) * exp(-tau_FF[i][j]);
            spec_BS_MSP_z1[i][j] = eps_BS_3( E_gam__GeV[j], n_H__cmm3[i], gso2D_BS, qe_MSP_z1_so ) * exp(-tau_FF[i][j]);
            spec_SY_MSP_z1[i][j] = eps_SY_4( E_gam__GeV[j], B__G[i], gso1D_SY, qe_MSP_z1_so ) * exp(-tau_FF[i][j]);
            spec_IC_MSP_z2[i][j] = eps_IC_3( E_gam__GeV[j], gso2D_IC, qe_MSP_z2_so ) * exp(-tau_FF[i][j]);
            spec_SY_MSP_z2[i][j] = eps_SY_4( E_gam__GeV[j], B_halo__G[i], gso1D_SY, qe_MSP_z2_so ) * exp(-tau_FF[i][j]);
...
*/
        ...
        gsl_so1D_free( gso_1D_Q_inject_MSP_z1 );
...
//    free2D( n_gal, q_p_SS_z1 );
    ...
    char filelist[20][21] = { "/spec_pi.txt", "/spec_IC_1_z1.txt", "/spec_IC_2_z1.txt", "/spec_BS_1_z1.txt", "/spec_BS_2_z1.txt", 
                              ...
                              "/spec_prompt_MSP.txt", "/spec_IC_MSP_z1.txt", "/spec_SY_MSP_z1.txt", "/spec_BS_MSP_z1.txt", 
                              "/spec_IC_MSP_z2.txt", "/spec_SY_MSP_z2.txt" };

    double **speclist[20] = { spec_pi, spec_IC_1_z1, spec_IC_2_z1, spec_BS_1_z1, spec_BS_2_z1, spec_SY_1_z1, spec_SY_2_z1,
                              spec_IC_1_z2, spec_IC_2_z2, spec_SY_1_z2, spec_SY_2_z2, spec_pi_fcal1, spec_nu, spec_FF,  
                              spec_prompt_MSP, spec_IC_MSP_z1, spec_SY_MSP_z1, spec_BS_MSP_z1, spec_IC_MSP_z2, spec_SY_MSP_z2 };
    ...
    char filelist_2[20][30] = { "/spec_pi_internal.txt", "/spec_IC_1_z1_internal.txt", "/spec_IC_2_z1_internal.txt", 
                                "/spec_BS_1_z1_internal.txt", "/spec_BS_2_z1_internal.txt", "/spec_SY_1_z1_internal.txt", 
                                "/spec_SY_2_z1_internal.txt", "/spec_IC_1_z2_internal.txt", "/spec_IC_2_z2_internal.txt", 
                                "/spec_SY_1_z2_internal.txt", "/spec_SY_2_z2_internal.txt", "/spec_pi_fcal1_internal.txt", 
                                "/spec_nu_internal.txt", "/spec_FF_internal.txt", "/spec_prompt_MSP_internal.txt", 
                                "/spec_IC_MSP_z1_internal.txt", "/spec_SY_MSP_z1_internal.txt", "/spec_BS_MSP_z1_internal.txt", 
                                "/spec_IC_MSP_z2_internal.txt", "/spec_SY_MSP_z2_internal.txt" };
    ...
    for (i = 0; i < n_gal; i++)
    {
        for (j = 0; j < n_E_gam; j++)
        {
            spectot[j] = (spec_pi[i][j] + spec_IC_1_z1[i][j] + spec_IC_2_z1[i][j] + spec_BS_1_z1[i][j] + spec_BS_2_z1[i][j] + 
                          spec_SY_1_z1[i][j] + spec_SY_2_z1[i][j] + spec_IC_1_z2[i][j] + spec_IC_2_z2[i][j] + spec_SY_1_z2[i][j] + 
                          spec_SY_2_z2[i][j] + spec_prompt_MSP[i][j] + spec_IC_MSP_z1[i][j] + spec_SY_MSP_z1[i][j] + 
                          spec_BS_MSP_z1[i][j] + spec_IC_MSP_z2[i][j] + spec_SY_MSP_z2[i][j]) * exp(-tau_gg[i][j]);
        }
